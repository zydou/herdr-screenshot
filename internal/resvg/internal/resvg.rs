...
#[no_mangle]
pub extern "C" fn usvg_options_default() -> *mut usvg::Options<'static> {
...
#[no_mangle]
pub extern "C" fn usvg_options_set_dpi(options: &mut usvg::Options<'_>, dpi: f32) {
    options.dpi = dpi;
}

#[no_mangle]
pub extern "C" fn usvg_options_set_font_family(options: &mut usvg::Options<'_>, family: *const c_char) -> *const c_char {
    let family = unsafe { CStr::from_ptr(family) };
    let family = match family.to_str() {
        Ok(v) => v.to_owned(),
        Err(e) => return CString::new(e.to_string()).unwrap().into_raw(),
...
#[no_mangle]
pub extern "C" fn usvg_options_set_font_size(options: &mut usvg::Options<'_>, font_size: f32) {
    options.font_size = font_size;
}

#[no_mangle]
pub extern "C" fn usvg_options_set_languages(options: &mut usvg::Options<'_>, languages: *const c_char) -> *const c_char {
...
#[no_mangle]
pub extern "C" fn usvg_options_set_shape_rendering_mode(options: &mut usvg::Options<'_>, mode: i32) {
...
#[no_mangle]
pub extern "C" fn usvg_options_set_text_rendering_mode(options: &mut usvg::Options<'_>, mode: i32) {
...
#[no_mangle]
pub extern "C" fn usvg_options_set_image_rendering_mode(options: &mut usvg::Options<'_>, mode: i32) {
...
#[no_mangle]
pub extern "C" fn usvg_options_set_default_size(options: &mut usvg::Options<'_>, width: f32, height: f32) {
...
/// Parses the SVG into a tree, converting text to glyphs along the way.
/// Takes ownership of the fontdb: since resvg 0.42 text shaping happens
/// during parsing and the tree owns the database. On return — success or
/// failure — the caller must treat the database pointer as consumed.
#[no_mangle]
pub extern "C" fn usvg_tree_from_data(data_ptr: *mut u8, data_size: usize, options: &mut usvg::Options<'_>, database: *mut fontdb::Database) -> Result<*mut usvg::Tree, *const c_char> {
    let data = unsafe { Vec::from_raw_parts(data_ptr, data_size, data_size) };
...
#[no_mangle]
pub extern "C" fn resvg_tree_render(tree: &mut usvg::Tree, transform: &mut tiny_skia::Transform, pixmap: &mut tiny_skia::Pixmap) {
    resvg::render(
        tree,
        *transform,
        &mut pixmap.as_mut(),

    ...
    for span in &chunk.spans {
        ...
        while let Some(new_glyph) = iter.next() {
            ...
            if prev_cluster_len < new_glyph.cluster_len {
                ...
                //
                // The fallback font may merge trailing clusters into fewer
                // glyphs than the primary font produced; guard the removal so
                // a ZWJ emoji sequence at the end of the run cannot panic.
                for k in 1..new_glyph.cluster_len {
                    if idx + k >= glyphs.len() {
                        break;
                    }
                    glyphs.remove(idx + k);

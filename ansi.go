...
// geom carries the fixed layout parameters the dispatcher needs.
// LineHeight must already be multiplied by Scale when the dispatcher runs
// (mirrors freeze's main.go), the background bar height depends on it.
type geom struct {
	Scale      float64
	FontSize   float64
	LineHeight float64
...
func (p *dispatcher) Print(r rune) {
	...
	if runewidth.RuneWidth(r) > 1 {
		...
		newChild.CreateAttr("dx", fmt.Sprintf("%.2fpx", (p.cfg.FontSize/5)*p.scale))
...
func (p *dispatcher) beginBackground(fill string) {
	...
	topOffset := ((p.cfg.FontSize + p.cfg.LineHeight) / 5) * p.scale
	rowMultiplier := p.cfg.FontSize * p.cfg.LineHeight

	y := fmt.Sprintf("%.2fpx", float64(p.row)*rowMultiplier+topOffset)
	x := p.scale * float64(p.col) * (p.cfg.FontSize / fontHeightToWidthRatio)
	rect.CreateAttr("x", fmt.Sprintf("%.2fpx", x))
	rect.CreateAttr("y", y)
	rect.CreateAttr("height", fmt.Sprintf("%.2fpx", p.cfg.FontSize*p.cfg.LineHeight+1))
	p.bg = rect
}
...
func (p *dispatcher) endBackground() {
	...
	p.bg.CreateAttr("width", fmt.Sprintf("%.5fpx", width*(p.cfg.FontSize/fontHeightToWidthRatio)))
